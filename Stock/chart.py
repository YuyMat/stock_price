...
class InitialPrice(object):
    # ランダムモード
    def initial_price(self, mode, price_history, initial_price):
        if mode == "r":
            rand_num = random.randint(90, 200)
            price_history.append(rand_num)
            return rand_num
        elif mode == "int":
            price_history.append(int(initial_price))
            return int(initial_price)
    
    
class FirstUpDown():    
    # 初期値の取得
    def __init__(self):
        # self.initial_price_instance = InitialPrice()
        # self.initial_price_instance.select_mode()
        # self.initial_price = self.initial_price_instance.initial_price()
        self.plus_minus_value = 0
        self.new_price = 0
    
    # 1度目の上がり(下がり)値の決定
    def up_down_value(self, initial_price, plus_minus_history_list):
        plus_minus_value = round(random.uniform(initial_price * 0.01, initial_price * 0.02), 2)
        ...
        plus_minus_history_list.append(plus_minus_value)
        return plus_minus_value
...
class ChartMovement():
    ...
    # plus_minus_valueがマイナスになるかプラスになるかの抽選
